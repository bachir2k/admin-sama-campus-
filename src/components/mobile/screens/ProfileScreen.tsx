  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: p.surface, border: `1px solid ${p.line}`, borderRadius: 18, padding: 16 }}>
        <div style={{ width: 58, height: 58, borderRadius: '50%', background: p.cardGrad, color: p.cardInk, display: 'grid', placeItems: 'center', fontFamily: DISP, fontWeight: 700, fontSize: 22, flexShrink: 0 }}>AN</div>
        <div style={{ flex: 1 }}>
        ...
        <span style={{ fontFamily: DISP, fontWeight: 600, fontSize: 12, color: p.brown, background: p.surfaceAlt, padding: '6px 11px', borderRadius: 999 }}>{STUDENT.id}</span>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: locked ? p.dangerSoft : p.okSoft, borderRadius: 16, padding: '13px 16px', marginTop: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: DISP, fontWeight: 700, fontSize: 14.5, color: locked ? p.danger : p.ok }}>{locked ? 'Carte verrouillée' : 'Carte active & protégée'}</div>
          <div style={{ fontSize: 12.5, color: p.ink2 }}>{locked ? 'Aucune transaction autorisée' : 'Détection de fraude par IA activée'}</div>
        </div>

  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <div style={{ flex: 1, background: p.cardGrad, color: p.cardInk, borderRadius: 18, padding: '16px 18px' }}>
        ...
        <div style={{ flex: 1, background: p.surface, border: `1px solid ${p.line}`, borderRadius: 18, padding: '16px 18px' }}>
          ...
          <div style={{ fontFamily: DISP, fontWeight: 700, fontSize: 30, color: p.ink }}>2<span style={{ fontSize: 18, color: p.muted }}>/4</span></div>
        ...
        {SCHEDULE.map((s, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, background: p.surface, border: `1px solid ${p.line}`, borderRadius: 16, padding: '14px 16px' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: DISP, fontWeight: 600, fontSize: 12.5, color: present ? p.ok : p.muted, background: present ? p.okSoft : p.surfaceAlt, padding: '6px 11px', borderRadius: 999 }}>
                {present ? <Icon name="check" size={14} color={p.ok} strokeWidth={2.6} /> : <Icon name="clock" size={14} color={p.muted} />}

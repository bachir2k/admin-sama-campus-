...
interface Props { p: Palette; mode?: 'pay' | 'recharge' }
...
export function PayScreen({ p, mode = 'pay' }: Props) {
  ...
  const mc = (k: typeof METHODS[0]['colorKey']) => ({ brown: p.brown, blue: p.blue, olive: p.olive }[k]);
    ...
    <div>
      ...
      <div style={{ display: 'flex', background: p.surfaceAlt, borderRadius: 13, padding: 4, marginBottom: 20 }}>
        {([['pay', 'Payer'], ['recharge', 'Recharger']] as ['pay'|'recharge', string][]).map(([k, l]) => (
          <button key={k} onClick={() => { setTab(k); setPaid(false); }} style={{ flex: 1, border: 'none', borderRadius: 10, padding: '10px 0', fontFamily: DISP, fontWeight: 600, fontSize: 14.5, background: tab === k ? p.surface : 'transparent', color: tab === k ? p.ink : p.muted }}>
            {l}
          </button>
        ))}
      </div>
      ...
      {tab === 'pay' ? (
        paid ? (
          <div style={{ textAlign: 'center', padding: '30px 0' }}>
            ...
            <button onClick={() => setPaid(false)} style={{ marginTop: 24, background: p.surface, border: `1px solid ${p.line}`, borderRadius: 999, padding: '12px 22px', fontFamily: DISP, fontWeight: 600, fontSize: 14.5, color: p.ink }}>Nouveau paiement</button>
          </div>
        ) : (
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ color: p.muted, fontSize: 14, marginTop: 4, maxWidth: 240, margin: '4px auto 0' }}>Présentez la carte ou le téléphone au terminal sans contact.</div>
            <button onClick={() => setPaid(true)} style={{ marginTop: 22, width: '100%', background: p.ink, color: p.surface, border: 'none', borderRadius: 14, padding: '15px 0', fontFamily: DISP, fontWeight: 700, fontSize: 15.5 }}>
        ...
        <div>
          <div style={{ textAlign: 'center', background: p.surface, border: `1px solid ${p.line}`, borderRadius: 18, padding: '22px 0' }}>
            <div style={{ fontSize: 12.5, color: p.muted, fontWeight: 600 }}>MONTANT À RECHARGER</div>
            <div style={{ fontFamily: DISP, fontWeight: 700, fontSize: 40, color: p.ink, marginTop: 4 }}><Money value={amount} /></div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 14 }}>
            {PRESETS.map(v => (
              <button key={v} onClick={() => setAmount(v)} style={{ border: `1px solid ${amount === v ? p.brown : p.line}`, background: amount === v ? p.surfaceAlt : p.surface, color: p.ink, borderRadius: 13, padding: '14px 0', fontFamily: DISP, fontWeight: 700, fontSize: 16 }}>
                <Money value={v} />
              </button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {METHODS.map((m, i) => (
              <button key={m.n} onClick={() => setMethod(i)} style={{ display: 'flex', alignItems: 'center', gap: 12, background: p.surface, border: `1px solid ${method === i ? p.brown : p.line}`, borderRadius: 14, padding: '13px 15px' }}>
                <span style={{ width: 34, height: 34, borderRadius: 9, background: mc(m.colorKey), display: 'grid', placeItems: 'center', color: '#fff', fontFamily: DISP, fontWeight: 700, fontSize: 15 }}>{m.n[0]}</span>

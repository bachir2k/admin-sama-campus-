...
export function AdminDashboard() {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', background: p.appBg2, fontFamily: BODY, color: p.ink }}>
      {/* Sidebar */}
      <aside style={{ width: 232, flexShrink: 0, background: p.nav, color: '#EDE7DB', display: 'flex', flexDirection: 'column', padding: '22px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '0 8px 22px' }}>
          <span style={{ width: 34, height: 34, borderRadius: 9, background: 'linear-gradient(135deg,#D7B477,#8B6B4A)', display: 'grid', placeItems: 'center' }}>
        ...
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.12em', color: 'rgba(237,231,219,.4)', padding: '0 10px 8px' }}>ADMINISTRATION</div>
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          {NAV.map(n => {
            const active = view === n.key;
            return (
              <button key={n.key} onClick={() => setView(n.key)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 12px', borderRadius: 11, border: 'none', cursor: 'pointer', background: active ? 'rgba(215,180,119,.16)' : 'transparent', color: active ? p.gold : 'rgba(237,231,219,.78)', fontFamily: DISP, fontWeight: 600, fontSize: 14.5, textAlign: 'left' }}>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: 11, padding: '12px 10px 0', borderTop: '1px solid rgba(237,231,219,.12)' }}>
          <span style={{ width: 36, height: 36, borderRadius: '50%', background: '#3a342a', display: 'grid', placeItems: 'center', fontFamily: DISP, fontWeight: 700, fontSize: 13, color: p.gold }}>SA</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: DISP, fontWeight: 700, fontSize: 13.5 }}>Service admin</div>
            <div style={{ fontSize: 11.5, color: 'rgba(237,231,219,.5)' }}>Campus central</div>
          </div>
          <Icon name="logout" size={18} color="rgba(237,231,219,.6)" />
        </div>
      </aside>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 28px', borderBottom: '1px solid ' + p.line, background: p.surface, flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button style={{ display: 'flex', alignItems: 'center', gap: 9, border: '1px solid ' + p.line, background: p.surface, borderRadius: 12, padding: '7px 9px 7px 7px', cursor: 'pointer' }}>
              <span style={{ width: 30, height: 30, borderRadius: 8, background: p.cardGrad, color: p.cardInk, display: 'grid', placeItems: 'center', fontFamily: DISP, fontWeight: 700, fontSize: 12 }}>SA</span>
              <Icon name="chevD" size={16} color={p.muted} />
            </button>
          </div>
        </header>
